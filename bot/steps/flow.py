...
def get_step_message(step: str, state: Optional[dict] = None) -> str:
    ...
    texts = {
        "1": (
            "Привет! Меня зовут Нерпа (Непрерывно Развиваю Процессы). 🦭 Я здесь, чтобы спокойно разобраться вместе с тобой.\n\n"
            "Посмотрим на процессы и команду, найдём варианты решений и проработаем риски — по шагам и без спешки. "
            "Результат будет понятным и полезным.\n\n"
            "Нажми «Анализ проблемы», когда будешь готов(а) начать."
        ),
        "2_upload": (
